...
public final class OnSubscribeConcatMap<T, R> implements OnSubscribe<R> {
    ...
    public void call(Subscriber<? super R> child) {
        ...
        child.setProducer(new ConcatMapProducer<>(parent));
    ...
    static final class ConcatMapProducer<T, R> implements Producer {
        private final ConcatMapSubscriber<T, R> parent;

        ConcatMapProducer(ConcatMapSubscriber<T, R> parent) {
            this.parent = parent;
        }
    ...
    static final class ConcatMapSubscriber<T, R> extends Subscriber<T> {
        ...
        void innerError(Throwable innerError, long produced) {
            ...
            } else if (delayErrorMode == IMMEDIATE) {
                Throwable ex = ExceptionsUtils.terminate(error);
                if (!ExceptionsUtils.isTerminated(ex)) {
                    actual.onError(ex);
                }
